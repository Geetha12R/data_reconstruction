...
def get_trainable_params(args, x0):
    n, c, h, w = x0.shape
    x = torch.randn(args.extraction_data_amount, c, h, w).to(args.device, dtype=torch.float) * args.extraction_init_scale
    x.requires_grad_(True)
    print(' trainable param ',x.shape, x0.shape)
    l = torch.rand(args.extraction_data_amount, 1).to(args.device, dtype=torch.float)
...
def get_kkt_loss(args, values, l, y, model, x):
    l = l.squeeze()
    kkt_loss = 0
    if args.num_classes == 2:
        ...
        for i, (p, grad) in enumerate(zip(model.parameters(), grad)):
            ...
            kkt_loss += l #stationary
        # print('kkt loss ',kkt_loss)
    else:
        phi_yi = values
        max_margin = torch.zeros_like(values, dtype=torch.float32, device=x.device)
        phi_j=[]
        for i in range(x.shape[0]):  
            phi_yi = values[i]  
            max_phi_j = torch.tensor(-float('inf'), device=phi_yi.device)  

            for j in range(len(x)): 
                if j!=i and (values[(j):] > max_margin[(j):]).all():
                    max_margin[j]= values[j]
            max_margin[i] = phi_yi - max_phi_j
        max_margin=max_margin[:,0]
        assert max_margin.dim() == 1
        assert l.dim() == 1
        assert y.dim() == 1
        assert max_margin.shape == l.shape == y.shape
        output = max_margin*l
        grad = torch.autograd.grad(
                outputs = output,
                inputs = model.parameters(),
                grad_outputs=torch.ones_like(max_margin, requires_grad=False, device=output.device).div(args.extraction_data_amount),
                create_graph=True,
                retain_graph=True,
            )
        for i, (p, grad) in enumerate(zip(model.parameters(), grad)):
...
def get_verify_loss(args, x, l):
    ...
    loss_verify += 5 * (-l + args.extraction_min_lambda).relu().pow(2).sum() #lambda loss
    return loss_verify
...
def calc_extraction_loss(args, l, model, values, x, y):
    ...
    if args.extraction_loss_type == 'kkt':
        kkt_loss = get_kkt_loss(args, values, l, y, model, x)
...
def evaluate_extraction(args, epoch, loss_extract, loss_verify, x, x0, y0, ds_mean):
    ...
    xx = x.data.clone()
    yy = x0.clone()
    dssim_score, dssim_grid = get_evaluation_score_dssim(xx, yy, ds_mean, vote=None, show=False, metric='dssim')
    # mssim_score, mssim_grid = get_evaluation_score_dssim(xx, yy, ds_mean, vote=None, show=False, metric='mssim')
    ...
    if args.wandb_active:
        wandb.log({
            ...
            # "extraction mssim": wandb.Image(mssim_grid),
