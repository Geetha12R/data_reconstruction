...
def get_total_err(args, p, y):
    if args.cls_type == 'multi':
        ...
        return 1 - (predicted == y).float().mean().item()
        # return err
    else:
...
def train(args, train_loader, test_loader, val_loader, model):
    optimizer = torch.optim.SGD(model.parameters(), lr=args.train_lr, momentum=args.momentum, weight_decay=args.weight_decay)
    print('Model:')
    print(model)
    ...
    for epoch in range(args.train_epochs + 1):
        ...
        if epoch % args.train_evaluate_rate == 0:
            ...
            if args.wandb_active:
                ...
                wandb.log({'epoch': epoch,'test loss': test_loss, 'test error': test_error})
...
def data_extraction(args, dataset_loader, model):
    ...
    if args.num_classes >2:
        # create labels (equal number of labels for all n classes)
        labels =[*range(10)]
        extraction = args.extraction_data_amount // args.num_classes
        for i in labels:
            y[i*extraction:((i+1)*extraction)] = i
    else: 
        y[:y.shape[0] // 2] = -1
        y[y.shape[0] // 2:] = 1
    y = y.long()
...
def setup_args(args):
    ...
    if args.wandb_active:
        wandb.init(project=args.wandb_project_name, entity='cnn-reconstruction')
